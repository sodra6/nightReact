...
// n 밀리세컨드 동안 기다리는 프로미스를 만들어 주는 함수
...
//방탄소년단 데이터 목록을 가져오는 비동기 함수
export const getBts = async () => {
    await sleep(500);//0.5s 쉬고 시작
    return bts;
}

//아이디로 방탄소년단 한명을 조회하는 비동기 함수
export const getBtId = async (id) => {
    await sleep(500); //0.5s 쉬고
    return bts.find(bt=> bt.id===id);//id 값으로 찾아서 한명 반환
}